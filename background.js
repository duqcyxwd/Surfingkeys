...
var ChromeService = (function() {
  ...
  chrome.webRequest.onErrorOccurred.addListener(
    function(details) {
      loadSettings('blacklist', function(data) {
        ...
        if (!disabled && tabId !== -1 && (conf.interceptedErrors.indexOf('*') !== -1 || conf.interceptedErrors.indexOf(details.error) !== -1)) {
  ...
  function _closeTab(s, n) {
    chrome.tabs.query({ currentWindow: true }, function(tabs) {
      ...
      chrome.tabs.remove(tabs.slice(s.tab.index + (n < 0 ? n : 1), s.tab.index + (n < 0 ? 0 : 1 + n)));
    });
  }

  self.closeTabLeft = function(message, sender, senderResponse) {
    _closeTab(sender, -message.repeats);
  };
  self.closeTabRight = function(message, sender, senderResponse) {
    _closeTab(sender, message.repeats);
  };
  self.closeTabsToLeft = function(message, sender, senderResponse) {
  ...
  self.deleteHistoryOlderThan = function(message, sender, sendResponse) {
    var days = message.days || 0,
      hours = message.hours || 0;

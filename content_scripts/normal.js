var Mode = (function() {
  ...
  self.handleMapKey = function(event, onNoMatched) {
    ...
    if (isEscKey && self.finish(this)) {
      ...
      event.sk_stopPropagation = !this.map_node.meta.stopPropagation || this.map_node.meta.stopPropagation(key);
      setTimeout(function() {
        pf(key);
        self.finish(thisMode);
        onAfterHandler(thisMode, event);
      }, 0);
    } else if (this.repeats !== undefined && this.map_node === this.mappings && runtime.conf.digitForRepeat && (key >= '1' || (this.repeats !== '' && key >= '0')) && key <= '9') {
    ...
    if (isEditable(realTarget) && event.isTrusted) {
      if (Mode.isSpecialKeyOf('<Esc>', event.sk_keyName)) {
        ...
        if (runtime.conf.editableBodyCare && realTarget === document.body && event.key !== 'i') {
          ...
          event.sk_stopPropagation = runtime.conf.editableBodyCare && realTarget === document.body && event.key === 'i';
          if (event.sk_stopPropagation) {
            realTarget.focus();
          }
    ...
    runtime.command(
      ...
      function(response) {
        ...
        if (elm === document.scrollingElement) {
          ...
          // visible rectangle
          var rc = [Math.max(br.left, 0), Math.max(br.top, 0), Math.min(br.right, window.innerWidth), Math.min(br.bottom, window.innerHeight)];
          ww = rc[2] - rc[0];
          wh = rc[3] - rc[1];
          sx = rc[0] * scale;
          sy = rc[1] * scale;

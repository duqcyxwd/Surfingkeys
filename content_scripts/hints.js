var Hints = (function() {
  ...
  self.addEventListener('keydown', function(event) {
    ...
    // if (event.keyCode === KeyboardUtils.keyCodes.enter) {
    //   // debugger;
    // }
  ...
  function refreshByTextFilter() {
    ...
    var cleanHint = function(hint) {
      hint.label = '';
      setInnerHTML(hint, '');
    };

    var hintMatch = function(hint) {
      var e = hint.link;
      ...
      var reg = new RegExp(textFilter, 'i');
  ...
  function createHintsForTextNode(rxp, attrs) {
    ...
    elements = positions
      .map(function(e) {
        ...
        if (e[0].data.trim().length === 0 || pos.top < caretViewport[0] || pos.left < caretViewport[1] || pos.top > caretViewport[2] || pos.left > caretViewport[3]) {
          return null;
        } else {

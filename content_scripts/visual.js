var Visual = (function() {
  ...
  self.mappings.add('p', {
    ...
    code: function() {
      ...
      while (p !== document.body) {
        ...
        if (range.comparePoint(textNodes[0], 0) === -1 || range.comparePoint(lastNode, lastNode.length) === 1) {
          self.hideCursor();
          state = 2;
          _onStateChange();
  ...
  function visualSeek(dir, chr) {
    ...
    if (selection.focusNode && selection.focusNode.textContent && selection.focusNode.textContent.length && selection.focusNode.textContent[selection.focusOffset] === chr && dir === 1) {

...
function map(new_keystroke, old_keystroke, domain, new_annotation) {
  if (_isDomainApplicable(domain)) {
    if (old_keystroke[0] === ':') {
      ...
      var keybound = createKeyTarget(
        function() {
          Front.executeCommand(cmdline);
        },
        new_annotation ? _parseAnnotation({ annotation: new_annotation }) : null,
        false
      );
...
function unmapAllExcept(keystrokes, domain) {
  if (_isDomainApplicable(domain)) {
    ...
    console.log('unmapAllExcept'); // REMOVETAG
  ...
  runtime.command(
    ...
    function(response) {
      ...
      document.addEventListener('mouseup', event => {
        if (runtime.conf.mouseSelectToQuery.indexOf(window.origin) !== -1 && !isElementClickable(event.target) && !event.target.matches('.cm-matchhighlight')) {

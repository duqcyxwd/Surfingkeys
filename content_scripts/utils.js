...
function getVisibleElements(filter) {
  ...
  for (var i = 0; i < all.length; i++) {
    ...
    if (rect.top <= window.innerHeight && rect.bottom >= 0 && rect.left <= window.innerWidth && rect.right >= 0 && rect.height > 0) {
...
function getClickableElements(selectorString, pattern) {
  var nodes = listElements(document.body, NodeFilter.SHOW_ELEMENT, function(n) {
    return n.offsetHeight && n.offsetWidth && getComputedStyle(n).cursor === 'pointer' && (n.matches(selectorString) || pattern.test(n.textContent));
  });
...
function filterInvisibleElements(nodes) {
  return nodes.filter(function(n) {
    return n.offsetHeight && n.offsetWidth && !n.getAttribute('disabled') && isElementPartiallyInViewport(n);
  });
}

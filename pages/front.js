var Front = (function() {
  ...
  _tabs.onShow = function(tabs) {
    ...
    tabs.forEach(function(t, i) {
      ...
      setInnerHTML(
        tab,
        `<div class=sk_tab_hint>${hintLabels[i]}</div><div class=sk_tab_wrap><div class=sk_tab_icon><img src='chrome://favicon/${t.url}'></div><div class=sk_tab_title>${htmlEncode(
          t.title
        )}</div></div>`
      );
      tab.url = t.url;
  ...
  function buildUsage(metas, cb) {
    ...
    initL10n(function(locale) {
      var help_groups = feature_groups.map(function() {
        return [];
      });
  ...
  function showRichHints(keyHints) {
    initL10n(function(locale) {
      ...
      words = Object.keys(cc)
        .sort()
        .map(function(w) {
          var meta = cc[w];
          if (meta.annotation) {
            return `<div><span class=kbd-span><kbd>${htmlEncode(KeyboardUtils.decodeKeystroke(keyHints.accumulated))}<span class=candidates>${w.substr(
              keyHints.accumulated.length
            )}</span></kbd></span><span class=annotation>${locale(meta.annotation)}</span></div>`;
          } else {
            return '';
          }
        })
    ...
    if (!_bubble.contains(e.target)) {
      ...
      var sel =
        window
          .getSelection()
          .toString()
          .trim() || Visual.getWordUnderCursor();
      if (sel && sel.length > 0) {
  ...
  _bubble.querySelector('div.sk_bubble_content').addEventListener(
    'mousewheel',
    function(evt) {

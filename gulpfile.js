...
gulp.task('lint', () => {
  return gulp
    .src(['gulpfile.js', 'content_scripts/*.js', 'pages/*.js', '!node_modules/**'])
    .pipe(eslint())
    .pipe(eslint.format())
...
gulp.task('clean', function() {
  return gulp.src(`dist/${buildTarget}-extension`, { read: false, allowEmpty: true }).pipe(clean());
});

gulp.task('copy-html-files', function() {
  if (buildTarget === 'Firefox') {
    return gulp
      .src(['pages/*.html', '!pages/pdf_viewer.html'], { base: '.' })
    ...
    return gulp.src(['pages/*.html'], { base: '.' }).pipe(gulp.dest(`dist/${buildTarget}-extension`));
  }
});

gulp.task('copy-non-js-files', function() {
  var nonJsFiles = ['icons/**', 'content_scripts/**', '!content_scripts/**/*.js', 'pages/**', 'libs/marked.min.js', '!pages/**/*.html', '!pages/**/*.js'];
  if (buildTarget === 'Firefox') {
    nonJsFiles.push('!pages/pdf/**');
  } else {
    nonJsFiles.push('libs/mermaid.min.js');
  }
  return gulp.src(nonJsFiles, { base: '.' }).pipe(gulp.dest(`dist/${buildTarget}-extension`));
});

gulp.task('copy-es-files', function() {
  return gulp
    .src(['content_scripts/front.js', 'content_scripts/content_scripts.js', 'content_scripts/top.js', 'pages/*.js'], { base: '.' })
...
gulp.task(
  'copy-js-files',
  gulp.series('copy-es-files', function() {
    var libs = ['libs/ace/*.js'];
...
gulp.task(
  ...
  gulp.series('copy-js-files', function() {
    return gulp.src(['pages/default.js'], { base: '.' }).pipe(gulp.dest(`dist/${buildTarget}-extension`));
  })
);

gulp.task('build_background', function() {
  var background = ['background.js'];
  if (buildTarget === 'Firefox') {
...
gulp.task(
  ...
  gulp.series('build_common_content_min_without_lib', function(cb) {
    if (buildTarget === 'Firefox') {
      return gulp
        .src([`dist/${buildTarget}-extension/content_scripts/common_content.min.js`, 'libs/shadydom.min.js'])
        .pipe(gp_concat('common_content.min.js'))
        .pipe(gulp.dest(`dist/${buildTarget}-extension/content_scripts`));
    } else {
      return gulp.src([`dist/${buildTarget}-extension/content_scripts/common_content.min.js`]).pipe(gulp.dest(`dist/${buildTarget}-extension/content_scripts`));
    }
  })
);
...
gulp.task(
  'build_manifest',
  gulp.series('copy-non-js-files', 'copy-html-files', function(cb) {
...
gulp.task('documentation:html', function() {
  ...
  return gulp
    .src('./{content_scripts/*.js,pages/*.js}')
...
gulp.task('deploy:docs', function() {
  return gulp.src('dist/documentation/**/*').pipe(ghPages());
});

gulp.task(
  'build',
  gulp.series('clean', gulp.parallel('lint', 'copy-pretty-default-js', 'build_common_content_min', 'build_manifest', 'build_background', 'documentation:md', 'documentation:html'), function() {
    return gulp
      .src(`dist/${buildTarget}-extension/**`)

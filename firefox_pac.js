...
function FindProxyForURL(url, host) {
  ...
  if (pacGlobal.proxyMode === 'always') {
    ...
    do {
      if (pacGlobal.hosts[0].hasOwnProperty(host) || (pacGlobal.autoproxy_pattern[0].length && pp.test(host))) {
        return 'DIRECT';
      }
    ...
    for (var i = 0; i < pacGlobal.proxy.length; i++) {
      ...
      var ahost = host;
      do {
        if (pacGlobal.hosts[i].hasOwnProperty(ahost) || (pacGlobal.autoproxy_pattern[i].length && pp.test(ahost))) {
